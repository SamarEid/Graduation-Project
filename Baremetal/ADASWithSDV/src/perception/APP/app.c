...
static ultraSonicInitTypeDef FRONT_ultraSonic   = {
		GPIOA,
		GPIOA,
		TIM2,
		TIM3,
...
static ultraSonicInitTypeDef LHS_ultraSonic 	= {
		GPIOB,
		GPIOB,
		TIM2,
		TIM4,
...
static ultraSonicInitTypeDef RHS_ultraSonic 	= {
		GPIOB,
		GPIOA,
		TIM2,
		TIM5,
...
static uint32_t LHS_firstCap = 0	, LHS_secondCap = 0,
			    RHS_firstCap = 0	, RHS_secondCap = 0,
				FRONT_firstCap = 0	, FRONT_secondCap = 0;
static uint8_t  LHS_flag = 0, RHS_flag, FRONT_flag;

//sensor data with its associated id
static sensorTypeDef FRONT_sensorData 	= {1,4.00};
static sensorTypeDef LHS_sensorData 	= {2,4.00};
static sensorTypeDef RHS_sensorData 	= {3,4.00};
...
void TIM3_IRQHandler(void){
	ultraSonicInputCaptureHandler(
			...
			&FRONT_firstCap,
			&FRONT_secondCap,
			(float*)(&(FRONT_sensorData)+sizeof(uint32_t)),
			&FRONT_flag);
}
...
void TIM4_IRQHandler(void){
	ultraSonicInputCaptureHandler(
			...
			&LHS_firstCap,
			&LHS_secondCap,
			(float*)(&(LHS_sensorData)+sizeof(uint32_t)),
			&LHS_flag);
}
...
void TIM5_IRQHandler(void){
	ultraSonicInputCaptureHandler(
			...
			&RHS_firstCap,
			&RHS_secondCap,
			(float*)(&(RHS_sensorData)+sizeof(uint32_t)),
			&RHS_flag);
}
...
void perceptionVidBegin(void){
	TaskHandle_t sendData = NULL;
	TaskHandle_t checkOverFlow = NULL;
	xTaskCreate(
